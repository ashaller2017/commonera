**Common Era — Vibe Coding Sprint
** July 10–11, 2026 · Los Angeles
This repo is the shared codebase for five prototypes built during the Common Era vibe coding sprint.
**How to use this repo
**Each project should in its own folder. Push your work to your folder only — don't touch anyone else's directory. IE: 
commonera/
  bmitzvah-coach/
  cosmic-calendar/
  digital-shabbat/

etc 

When you're done, your folder should contain your full project code plus a README.md with a brief description of what you built, what works, what's rough, what tools/models you used, and what you'd do next.

**Deliverables checklist
**
 -Latest code pushed to your project folder
 -README.md inside your folder
 -Prototype deployed + link (should work for minimum 1 month) in your README
 -Video walkthrough uploaded to the shared Google Drive
 -Your slides filled out in the shared Google Slides deck

Questions? Reach out to Kris — kbudelis@gmail.com
